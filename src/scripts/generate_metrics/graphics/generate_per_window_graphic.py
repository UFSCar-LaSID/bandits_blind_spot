...
def generate_per_window_graphic(save_path: str, calcultated_metrics: MetricsDict):
    '''
    Gera e salva os gráficos das médias por janela (para todas as métricas, tanto html, png e csv).

    O gráfico terá linhas com o desempenho dos algoritmos ao longo das janelas (não é agregado, as métricas são calculadas unicamente para cada janela). OBS: Um gráfico desse é gerado para cada tipo de métrica selecionado.
    ...
    params:
        save_path: Pasta raiz usada para salvar os arquivos previamente citados.
    ...
    for metric_name, metric_dict in calcultated_metrics['metrics'].items():
        ...
        per_window_df = metric_dict['per_window']
        algo_names = per_window_df['algo_name'].unique()
        ...
        for algo_name, config in algos_configs.items():
            df_algo = per_window_df[per_window_df['algo_name'] == algo_name]
        ...
        fig.write_html(os.path.join(save_path, 'graphics', metric_name, src.FILE_PER_WINDOW_GRAPHIC_HTML))
        fig.write_image(os.path.join(save_path, 'graphics', metric_name, src.FILE_PER_WINDOW_GRAPHIC_PNG))

        save_dataframe(
            per_window_df,
            os.path.join(save_path, 'graphics_tables', metric_name, src.FILE_PER_WINDOW_TABLE)
        )